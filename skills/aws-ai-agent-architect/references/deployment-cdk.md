...
| `@aws-cdk/aws-bedrock-alpha` — `Agent.idleSessionTTL` | `Duration`. How long sessions stay open. Default is **10 minutes** (NOT 1 hour as reported in older documentation). | `Duration.minutes(10)` (verified default) |
| `@aws-cdk/aws-bedrock-alpha` — `AgentAlias` props (verified) | Constructor props: `agent` (required `IAgent`), `agentAliasName` (optional, default `'latest'`), `agentVersion` (optional, creates new version by default), `description` (optional). `routingConfiguration` does **not** exist as a constructor prop. | `new bedrock.AgentAlias(this, 'myAlias', { agent, agentAliasName: 'production', description: \`...${agent.lastUpdated}\` })` |
...
- `Agent.idleSessionTTL` default is **10 minutes**, not 1 hour as often reported in third-party documentation.

- `AgentAlias` has no `routingConfiguration` prop in its constructor. The verified constructor props are: `agent`, `agentAliasName`, `agentVersion`, `description`.
...
- [class AgentAlias (construct) — aws-bedrock-alpha API reference](https://docs.aws.amazon.com/cdk/api/v2/docs/@aws-cdk_aws-bedrock-alpha.AgentAlias.html) — Verified props: `agent` (required), `agentAliasName` (optional), `agentVersion` (optional), `description` (optional). No `routingConfiguration` in constructor props.
